...
def parse_table_data_new(table: list[list[str]], person_template: dict, start_row: int = 0) -> list[dict]:
    ...
    for row in table[start_row:]:
        person = deepcopy(person_template)

        for field in person.values():
            if field["table_index"] < 0:
                field["value"] = ""
            elif field["table_index"] < len(row):
                field["value"] = field["table_index"].strip()
            else:
                raise

        split_name = person["full_name"]["value"].split(" ")
        if len(split_name) == 3:
            person["last_name"]["value"] = split_name[0]
            person["first_name"]["value"] = split_name[1]
            person["middle_name"]["value"] = split_name[2]
        persons.append(person)
...
def add_data_to_persons_list_new(persons_list: list[dict], data_type:str, data: str): 
    for person in persons_list:
        if data_type in person.keys():
            person[data_type]["value"] = data
 


def extract_docx_table(docx_path: str, table_index: int) -> list[list[str]] | None:
