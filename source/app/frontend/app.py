...
class App(tk.Tk):
    ...
    def show_license_dialog(self):
        ...
        key_frame.pack(padx=20, pady=(5, 0), fill="x")
        ...
        file_btn_frame = tk.Frame(dialog)
        file_btn_frame.pack(padx=20, pady=(8, 15), fill="x")
        ...
        def open_license_file():
            ...
            if file_path:
                try:
                    ...
                    key_entry.delete(0, tk.END)
                    key_entry.insert(0, license_content)
                except Exception as e:
                    messagebox.showerror("Ошибка", f"Не удалось прочитать файл:\n{e}", parent=dialog)

        ttk.Button(
            file_btn_frame,
            text="📁 Открыть файл с ключом",
            command=open_license_file
        ).pack(anchor="center")
