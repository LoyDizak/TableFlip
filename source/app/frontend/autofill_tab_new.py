...
from backend.web import open_browser, confirm_entry, fill_person_form_new
from backend.json_handling import load_from_json
from backend.string_converter import persons_list_to_string_new, person_to_string_new
...
class AutofillTab:
    ...
    def update_json_preview(self):
        ...
        self.autofill_preview.insert(tk.END, persons_list_to_string_new(self.persons, self.template))
    ...
    def update_current_person_preview(self):
        ...
        if not self.persons:
            ...
            self.current_person_preview.insert(tk.END, person_to_string_new(current_person, self.template))
    ...
    def on_button_fill_info(self):
        ...
        if not self.persons:
            ...
            return
        try:
            fill_person_form_new(self.web_driver, self.persons[self.current_person_index], self.template)
        except Exception:
