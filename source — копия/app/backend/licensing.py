...
class LicenseSystem:
    ...
    def __init__(self, app_name: str, public_key: str):
        ...
        print(self.date_check_file_path)
    ...
    def get_hardware_id(self) -> str:
        ...
        try:
            if platform.system() == "Windows":
                # Используем UUID материнской платы
                cmd = "wmic csproduct get uuid"
                result = subprocess.check_output(cmd, shell=True).decode()
                hwid = result.split('\n')[1].strip()
            else:
                # Для Linux/Mac - используем machine-id
                hwid = subprocess.check_output(["cat", "/etc/machine-id"]).decode().strip()
            
            return hashlib.sha256(hwid.encode()).hexdigest()[:16]
        except:
            # Fallback - используем hostname и username
            try:
                fallback = f"{platform.node()}-{os.getlogin()}"
            except:
                fallback = f"{platform.node()}-unknown"
            return hashlib.sha256(fallback.encode()).hexdigest()[:16]


    def set_license_key(self, new_key: str):
        # Простейшая валидация PEM
        if "BEGIN" not in new_key or "PRIVATE KEY" not in new_key:
            raise ValueError("Key does not look like a PEM private key")
